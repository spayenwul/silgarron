...
| **Погода** | Атмосфера | Метаболические циклы + "Вдох"/"Выдох" |

### Система "Дыхания" мира

**Из лора:** Весь мир участвует в циклическом "Дыхании" — расширении и сжатии поверхности.

**Ключевая механика:**
- **Вдох**: Воздух втягивается в **Альвеолярные каверны** (аналог лёгких)
- **Тяга создаёт воздушные потоки** от поверхности вниз
- **Влажность высушивается** на пути к "лёгким"

**Это означает:**
- Влажность зависит НЕ только от близости к лимфотокам, но и от **линий воздушного потока**
- Нужно моделировать "каверны-аттракторы" и потоки к ним
...
1. **Работающий анатомический генератор** (`core/world_generator.py`)
   ...
   - Система "Дыхания" (BFS для воздушных потоков к кавернам)
   - Метаболическое тепло (активность тканей)

2. **Модели данных** (`models/global_map.py`)
   - `GlobalSector` с **анатомическими свойствами** (tissue_type, metabolic_heat, airflow_intensity)
...
3. **Визуализатор медицинского сканирования** (`tools/visualize_global_map.py`)
   - 5 слоёв: Structural Anatomy, Tissue Morphology, Metabolic Heatmap, Fluid Saturation, Airflow Map
